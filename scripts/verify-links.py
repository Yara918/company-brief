...
def main():
    ...
    # ===== 内容校验（company-brief 专用） =====
    content_issues = []
    with open(report_path, "r", encoding="utf-8") as f:
        html = f.read()

    # 1. 模板残留文字：报告里不允许出现占位符/模板说明残留（「这单合作值不值得谈」是 Fit 的正式文案，允许）
    for banned in ["{{", "}}", "（示例", "模板", "TODO", "占位"]:
        if banned in html:
            content_issues.append(f"发现模板残留文字: {banned}")

    # 2. KPI 必须是大数字/数量：速读卡 3 个关键数字的值不能是纯文字结论
    #    提取 .kpi .v 的内容，检查是否含数字
    kpi_values = re.findall(r'<div class="v">([^<]{1,40})</div>', html)
    for v in kpi_values:
        clean = re.sub(r"<[^>]+>", "", v).strip()
        if clean and not re.search(r"[0-9０-９一二三四五六七八九十百千万亿]", clean):
            content_issues.append(f"KPI 值不是数字（疑似文字结论）: {clean}")

    # 3. Fit 徽章必须是 强/中/弱，且理由存在
    fit_badges = re.findall(r'class="fit[^"]*">([^<]{1,10})</span>', html)
    for b in fit_badges:
        if b not in ("强", "中", "弱") and not b.startswith(("强", "中", "弱")):
            content_issues.append(f"Fit 徽章不是 强/中/弱: {b}")
    if "fit-reason" not in html and "fit_reason" not in html:
        # fit 理由缺失时告警（不阻断，理由可简写）
        pass

    # 4. 社媒表操作列检查：第 3 列只放「进入主页」按钮或留空，禁止 srclink / 「未检索到」/ 长文字
    ...
    for sm_body in sm_rows_html:
        ...
        for tr in sm_trs:
            ...
            if len(tds) >= 3:
                ...
                if "srclink" in op:
                    content_issues.append("社媒操作列出现 srclink（应只放进入主页按钮或留空）")
    ...
    # 5. 敏感词检查：报告不允许出现敏感词（命中即 FAIL，列出上下文供模型修正）
    SENSITIVE_WORDS = (
    ...
    # 6. HTML 结构检查：容器标签开闭不平衡会导致排版变形（div 多/少闭合等）
